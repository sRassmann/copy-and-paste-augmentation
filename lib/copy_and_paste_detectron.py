...
class CAP_Dataset(Dataset):
    ...
    def __init__(self, 
        ...
        if collection_box_cfg.P > 0:
            self.collection_box_cpg = CollectionBoxGenerator(
                ...
                scale_augment_dict=self.get_pool_cfg(pools_cfg, collection_box_cfg.POOL_NAME),
        ...
        if random_cap_cfg.P > 0:
            self.random_cpg = RandomGenerator(
                self.patch_pool, 
                self.background_pool,
                scale_augment_dict=self.get_pool_cfg(pools_cfg, random_cap_cfg.POOL_NAME),
    ...
    def get_pool_cfg(cfg_path, pool_key):
        with open("configs/insects_cap_pool_config.yaml", "r") as y:
    ...
    def init_base_patch_pool(obj_dir):
        """
        initialize pool of raw object patches
        """
    ...
    def __getitem__(self, idx):
        # distribute prob between random etc
        np.random.seed(idx)
        ...
        instances = detection_utils.annotations_to_instances(anns, image.shape[:2], "bitmask")
    ...
    def show_image_and_mask(dataset_dict):
        ...
        if len(dataset_dict["instances"].gt_boxes):
           cv2_imshow(CAP_Dataset.get_projected_mask(dataset_dict)*255)

class Trainer(DefaultTrainer):
    """
    Trainer constructing and using a CapDataset from the config
    """
    # TODO use existing set
    @classmethod
    ...
    def build_train_loader(cls, cfg):
        dataset = CAP_Dataset(cfg)
        train_loader = build_detection_train_loader(dataset, total_batch_size=1, mapper=None)
        return train_loader